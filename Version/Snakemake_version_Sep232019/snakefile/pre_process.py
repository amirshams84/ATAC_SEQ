		...
		run:
			for each_fastq in input.fastq_List:
				...
				shell("""
					#
					##
					DATA_PATH={WORKDIR}/{PROJECT}/{EXPERIMENT}/{TITLE}/{GENOME}/{wildcards.design}/pre_process
					mkdir -p $DATA_PATH
					...
					printf "%s\\n" "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" | tee >(cat >&2)
					printf "%s\\n" "module load cutadapt/2.1 || exit 1"  | tee >(cat >&2)
					printf "%s\\n" "#" | tee >(cat >&2)
					printf "%s\\n" "cutadapt {config_pre_process_Dict[CUTADAPT_PAIRED]} --cores={threads} --output={output.processed_fwd_fastq}.tmp \\
					--paired-output={output.processed_rev_fastq}.tmp {each_fastq} {each_fastq_reverse} > $QC_PATH/{each_fastq_begining}.cutadapt.txt" | tee >(cat >&2)
					...
					{each_fastq} {each_fastq_reverse} > $QC_PATH/{each_fastq_begining}.cutadapt.txt
